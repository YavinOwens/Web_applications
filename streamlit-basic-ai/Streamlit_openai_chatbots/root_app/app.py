...
def overview_page():
    ...
    with st.container():
        ...
        with col1:
            ...
            appt_by_hour['predicted'] = appt_by_hour['count'] * np.random.uniform(0.9, 1.1, size=len(appt_by_hour))
            appt_by_hour['forecast'] = appt_by_hour['count'] * np.random.uniform(0.8, 1.2, size=len(appt_by_hour))
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=appt_by_hour['hour'], y=appt_by_hour['count'], mode='lines+markers', name='Actual'))
            fig.add_trace(go.Scatter(x=appt_by_hour['hour'], y=appt_by_hour['predicted'], mode='lines+markers', name='Predicted'))
            fig.add_trace(go.Scatter(x=appt_by_hour['hour'], y=appt_by_hour['forecast'], mode='lines+markers', name='Forecast'))
            fig.update_layout(title='Number of Appointments by Hour', xaxis_title='Hour', yaxis_title='Number of Appointments')
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            st.subheader('Appointments Table')
            st.dataframe(appt_by_hour)
    ...
    with st.expander('Current Waiting Appointments'):
        ...
        with col1:
            st.subheader('Waiting Time Line Graph')
            waiting_appts['hour'] = waiting_appts['appointment_date'].dt.hour
            waiting_time_summary = waiting_appts.groupby('hour').agg(
                median_waiting_time=('waiting_time', 'median'),
                min_waiting_time=('waiting_time', 'min'),
                max_waiting_time=('waiting_time', 'max')
            ).reset_index()
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=waiting_time_summary['hour'], y=waiting_time_summary['median_waiting_time'], mode='lines+markers', name='Median Waiting Time'))
            fig.add_trace(go.Scatter(x=waiting_time_summary['hour'], y=waiting_time_summary['min_waiting_time'], mode='lines+markers', name='Min Waiting Time', line=dict(dash='dash')))
            fig.add_trace(go.Scatter(x=waiting_time_summary['hour'], y=waiting_time_summary['max_waiting_time'], mode='lines+markers', name='Max Waiting Time', line=dict(dash='dash')))
            fig.update_layout(title='Waiting Time for Appointments by Hour', xaxis_title='Hour', yaxis_title='Waiting Time (minutes)')
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            st.subheader('Waiting Appointments Table')
            st.dataframe(waiting_appts_display)

        # median_waiting_time = waiting_appts['waiting_time'].median()
        # st.subheader('Appointment Waiting Times')
        # for index, row in waiting_appts.iterrows():
        #     delta = row['waiting_time'] - median_waiting_time
        #     arrow = "🔺" if delta > 0 else "🔻"
        #     delta_color = "inverse" if delta > 0 else "normal"
        #     st.metric(label=f"Appointment {row['appointment_id']}", value=row['waiting_time'], delta=f"{arrow} {abs(delta)}", delta_color=delta_color)
...
def workforce_management_page():
    ...
    with st.expander('Case Allocations'):
...
def data_quality_page():
    ...
    with st.sidebar:
        st.header('Filter')
        selected_data_file = st.selectbox('Choose Data File', [
            'hospitals.csv',
            'clinics.csv',
            'providers.csv',
            'patients.csv',
            ...
            'medical_assets.csv',
            'workforce.csv',
            'case_allocations.csv'
        ])
    
    # Load selected data file
    data_path = f'data/{selected_data_file}'
    data = load_data(data_path)

    # Data Quality Metrics
    st.subheader('Completeness')
    completeness_metrics = data.notna().mean().reset_index()
    completeness_metrics.columns = ['Column', 'Completeness']
    st.table(completeness_metrics)

    st.subheader('Uniqueness')
    uniqueness_metrics = data.nunique().reset_index()
    uniqueness_metrics.columns = ['Column', 'Uniqueness']
    st.table(uniqueness_metrics)

    st.subheader('Consistency')
    # For simplicity, assume consistency is calculated as the number of duplicates
    duplicates = data.duplicated().sum()
    consistency_metrics = pd.DataFrame({
        'Metric': ['Duplicates'],
        'Count': [duplicates]
    })
    st.table(consistency_metrics)

    st.subheader('Accuracy and Validity')
    st.write("To check for accuracy and validity, domain knowledge is required to define valid ranges or sets of values for each column. This is a placeholder to remind you that these checks need to be implemented as per the specific data domain requirements.")

def data_privacy_page():
    st.header('Data Privacy')

    st.subheader('Data Privacy Considerations')
    st.write("""
    In this healthcare data application, data privacy is of utmost importance. Here are some key considerations to ensure data privacy:
    - **Anonymization**: Ensure that any patient data used is anonymized to protect personal information.
    - **Access Control**: Limit access to sensitive data based on user roles and responsibilities.
    ...
    - **Audit Trails**: Maintain audit trails of data access and modifications to ensure accountability.
    """)

    st.subheader('Data Anonymization')
    st.write("""
    When generating synthetic healthcare data, it's essential to anonymize personal information to prevent identification of individuals. This includes:
    - Removing or encrypting identifiers such as names, addresses, and phone numbers.
    - Aggregating data to higher levels where individual identification is not possible.
    """)

    st.subheader('Data Security Measures')
    st.write("""
    Implement the following measures to ensure data security:
    ...
    - **Secure Data Disposal**: Ensure that data is securely disposed of when no longer needed.
    """)

    st.subheader('User Access Management')
    st.write("""
    Manage user access to data through the following practices:
    - **Role-Based Access Control (RBAC)**: Implement RBAC to ensure users have access only to the data necessary for their roles.
    - **Regular Access Reviews**: Conduct regular reviews of user access to ensure appropriateness.
    - **Least Privilege Principle**: Grant users the minimum level of access required to perform their duties.
    """)

# Page navigation
page = st.sidebar.selectbox("Select Page", ["Overview", "Workforce Management", "Patient Management", "Asset Management", "Data Quality", "Data Privacy"])
